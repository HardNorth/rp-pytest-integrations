...
@step("Custom name nested step", params=[])
def custom_name():
    print('Inside nested step with custom name')


def test_function_level_nested_steps_start_stop(rp_logger):
    nested_step()
    ...
    with step("Code level nested step", {"test": "value"}):
        rp_logger.info("Inside code-level nested step")

    with step("Code level nested step with custom status", status='FAILED'):
        rp_logger.info("Inside code-level nested step")
